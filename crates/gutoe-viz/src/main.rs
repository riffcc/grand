...
use renderer::{
    CameraUniforms, InstanceData, Renderer,
    hex_world_pos, look_at, mat4_mul, perspective, state_color,
    COL_STEP, LAYER_SPACING, ROW_STEP,
};
...
impl App {
    fn new() -> Self {
        ...
        let world_positions: Vec<[f32; 3]> = (0..n).map(|site| {
    ...
    fn simulation_step(&mut self) {
        if self.state.step_count == PHASE1_STEPS && !self.injected {
            ...
            self.state.phase = 2;
            self.injected = true;
            log::info!("Phase 2: injected {} leptons, {} proton triplets",
                       N_INJECT, triplets.len());
        }
        ...
        if self.state.phase == 2 && self.state.step_count % GAUGE_EVERY == 0 {
            ...
            update_gauge(
                &mut self.gauge, &self.state.lattice, &q_map,
                &self.proton_sites, &self.cfg, &self.nbr_cache,
            );

            self.n_protons = triplets.len();
            self.n_leptons = self.state.lattice.iter()
                .filter(|&&s| s == sim::LEPTON_SEED).count();
    ...
    fn build_instances(&self) -> Vec<InstanceData> {
        ...
        (0..n).map(|site| {
            ...
            InstanceData { world_pos: wpos, _pad: 0.0, color: col }
        }).collect()
    }
    ...
    fn window_title(&self) -> String {
        format!(
            ...
            self.state.phase, self.state.step_count,
            self.n_protons, self.n_leptons, self.n_hydrogen, self.enrichment,
        )
    }
}
...
impl ApplicationHandler for App {
    fn resumed(&mut self, el: &ActiveEventLoop) {
        ...
        let renderer = pollster::block_on(Renderer::new(
            &wgpu_instance, surface, size.width, size.height, n,
        ));
        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                if self.mouse_pressed {
                    ...
                    self.camera.elevation  = (self.camera.elevation - dy * 0.004)
                        .clamp(0.05, 1.50);
                }
            ...
            WindowEvent::MouseWheel { delta, .. } => {
                ...
                self.camera.distance = (self.camera.distance * 0.9_f32.powf(scroll))
                    .clamp(40.0, 1200.0);
            }
