...
pub fn two_electron_ground_state(
    ...
    for _ in 0..n_selfcons {
        ...
        let phi_eff1: Vec<f64> = phi_proton.iter().zip(phi_e2.iter())
            .map(|(&p, &e)| p + e).collect();
        for _ in 0..n_iter {
        ...
        let phi_eff2: Vec<f64> = phi_proton.iter().zip(phi_e1.iter())
            .map(|(&p, &e)| p + e).collect();
        for _ in 0..n_iter {
    ...
    let rho_e2: Vec<f64> = psi2.iter().map(|a| charge * a.norm_sqr()).collect();
    let phi_e2 = jacobi_poisson(&rho_e2, cfg, n_jacobi);
    let phi_eff1: Vec<f64> = phi_proton.iter().zip(phi_e2.iter()).map(|(&p, &e)| p + e).collect();
    let (e1, _, _) = expected_energy(&psi1, &phi_eff1, cfg, charge, alpha_em);

    let rho_e1: Vec<f64> = psi1.iter().map(|a| charge * a.norm_sqr()).collect();
    let phi_e1 = jacobi_poisson(&rho_e1, cfg, n_jacobi);
    let phi_eff2: Vec<f64> = phi_proton.iter().zip(phi_e1.iter()).map(|(&p, &e)| p + e).collect();
    ...
    let e_int: f64 = psi1.iter().zip(phi_e2.iter())
...
pub fn measure_z3_instanton_rate(
    ...
    let mut prev_sets: Vec<[usize; 3]> = {
        ...
        crate::analysis::find_proton_triplets(&quarks, cfg)
            .iter()
            .map(|t| { let mut k = *t; k.sort_unstable(); k })
            .collect()
    };
    ...
    for step_idx in 0..n_measure {
        lat = crate::sim::step(
            &lat, &mut rng, cfg, None, &Default::default(), n_phase1 + step_idx,
        );
        ...
        for prev in &prev_sets {
            ...
            let hopped = cur_sets.iter().any(|cur| {
                cur.iter().filter(|&&s| prev_sites.contains(&s)).count() == 2
            });
    ...
    let rate   = if n_tracked > 0 { n_hops as f64 / n_tracked as f64 } else { 0.0 };
    let s_inst = if rate > 0.0 { -rate.ln() } else { f64::INFINITY };
...
mod tests {
    ...
    fn lattice_fermion_propagator_free() {
        let cfg = LatticeConfig { hex_rows: 12, hex_cols: 12, layers: 1, ..Default::default() };
        let n = cfg.n_sites();
        let source = n / 2;
        let dtau = 0.08;
        let n_steps = 300;
        ...
        println!("G(0) = {:.4}, G({n_steps}) = {:.6}", g_series[0], g_series[n_steps]);

        // G starts at 1 and decays
        assert!((g_series[0] - 1.0).abs() < 1e-10, "G(0) should be 1 by construction");
        ...
        assert!(
            ...
            "G should decay significantly: G({n_steps}) = {:.4}", g_series[n_steps]
        );

        // Mass gap: m_eff in (0, 0.5) for the hex Laplacian
        assert!(m_eff > 0.0, "Mass gap m_eff should be positive, got {m_eff:.6}");
        assert!(m_eff < 0.5, "m_eff={m_eff:.4} should be < 0.5 for 12×12 hex Laplacian");

        println!(
            "MASS GAP CONFIRMED: E₁ = {m_eff:.5}  (hex lattice free propagator)"
        );
    }
    ...
    fn coulomb_propagator_bound_state_pole() {
        ...
        println!(
            "Free: C(0)={c_i_free:.4} → C(50)={c_f_free:.4}  grew={grew_free}"
        );
        println!(
        ...
        assert!(grew_coulomb, "Coulomb propagator should grow (bound state E < 0)");
    ...
    fn two_electrons_repel() {
        ...
        let (_, e_single, _, _) = quantum_hydrogen_ground_state(
            &phi_proton, &shell, &cfg, 300, 0.05, alpha_em,
        );
        ...
        println!("  Repulsion shift:    ΔE = E₁ − E_single = {:+.6}", e1 - e_single);
    ...
    fn two_electron_density_anticorrelation() {
        ...
        let (psi1, psi2, _e1, _e2, e_int) = two_electron_ground_state(
            &phi_proton, &cfg, 150, 5, 300, 0.05, alpha_em,
        );

        // Co-location probability S = Σ_i |ψ₁(i)|²|ψ₂(i)|²
        let s_coloc: f64 = psi1.iter().zip(psi2.iter())
        ...
        let shell = vec![center];
        let (psi0, _, _, _) = quantum_hydrogen_ground_state(
            &phi_proton, &shell, &cfg, 300, 0.05, alpha_em,
        );
        ...
        println!("  Ratio S/S₀ = {:.4} (< 1 means electrons avoid each other)", s_coloc / s_no_repel);
        ...
        assert!(e_int > 0.0, "Interaction energy must be positive: {e_int:+.6}");
    ...
    fn deficit_angle_pentagon_is_pi_over_3() {
        ...
        assert!(deficit_angle(6).abs() < 1e-12, "Regular hex has zero curvature");
        let delta7 = deficit_angle(7);
        assert!(
            (delta7 + expected).abs() < 1e-12,
            "Heptagon deficit angle: got {delta7}, expected -π/3 = {}", -expected
        );
    ...
    fn gravity_from_disclination_binds_lepton() {
        ...
        let (psi_grav, e_grav, e_kin, e_pot) = gravity_from_defect(
            defect, k_pentagon, &cfg, 500, 300, 0.05, alpha_g,
        );
        ...
        let (_, e_em, _, _) = quantum_hydrogen_ground_state(
            &phi_em, &shell, &cfg, 300, 0.05, alpha_g,
        );
        ...
        println!("    Gravity  (K=π/3):    E = {:+.6} (kin={:+.6}, pot={:+.6})", e_grav, e_kin, e_pot);
    ...
    fn h2_potential_curve() {
        ...
        let l = 24usize;
        let cfg = LatticeConfig { hex_rows: l, hex_cols: l, layers: 1, ..Default::default() };
        let alpha_em = 1.0_f64;
        let n_jacobi = 500;
        let n_iter = 400;
        let dtau = 0.04;
        ...
        let (_, e_isolated, _, _) = quantum_hydrogen_ground_state(
            &phi_single, &shell, &cfg, n_iter, dtau, alpha_em,
        );
        ...
        for &sep in &separations {
            ...
            let (e_e, e_pp, e_tot) = h2_plus_energy(
                p1, p2, &cfg, n_jacobi, n_iter, dtau, alpha_em,
            );
            let delta_e = e_tot - e_isolated;
            println!("  {:>4}  {:>10.6}  {:>10.6}  {:>10.6}  {:>+8.4}",
    ...
    fn entropy_extremes() {
        ...
        assert!(s_delta.abs() < 1e-10, "Delta entropy should be 0, got {s_delta}");
    ...
    fn arrow_of_time_free_vs_bound() {
        ...
        println!("  FREE (V=0):     S: {s_free_initial:.4} → {s_free_final:.4}  ΔS={:+.4}", s_free_final - s_free_initial);
        println!("  BOUND (Coulomb): S: {s_bound_initial:.4} → {s_bound_final:.4}  ΔS={:+.4}", s_bound_final - s_bound_initial);
    ...
    fn entropy_extensivity() {
        ...
        println!("  {:>6}  {:>6}  {:>10}  {:>10}", "rows", "cols", "S_uniform", "ln(n)");

        for (rows, cols) in sizes {
            let cfg = LatticeConfig { hex_rows: rows, hex_cols: cols, layers: 1, ..Default::default() };
            let n = cfg.n_sites();
            let a = 1.0 / (n as f64).sqrt();
    ...
    fn z3_instanton_rate_measurement() {
        ...
        let (rate, s_inst, n_hops, n_tracked) =
            measure_z3_instanton_rate(&cfg, 200, 500, 42);
        ...
        assert!(n_tracked > 0, "Must have tracked at least some triplet-steps");
    ...
    fn z3_instanton_rg_matches_mass_ratio() {
        use crate::sim::{z3_instanton_action, instanton_threshold, landau_pole};
        ...
        assert!(s_before < s_pred, "S_inst(t*-1)={s_before:.4} should be < {s_pred:.4}");
        assert!(s_at >= s_pred,    "S_inst(t*)={s_at:.4} should be ≥ {s_pred:.4}");
    ...
    fn topological_charge_accumulator() {
        ...
        for _ in 0..measure_steps {
            let eligible = lat.iter()
                .filter(|&&s| s != crate::config::VOID && s != crate::config::LEPTON_SEED)
                .count();
            total_eligible += eligible;

            let (next, cycles) = step_counted(&lat, &mut rng, &cfg, None, &Default::default(), t_measure);
            total_cycles += cycles;
            lat = next;
        }
