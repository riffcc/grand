//! Lithium-7 tension report: baseline vs structural corrected lane.

use gutoe_physics::bbn::{
    evaluate_bbn_gate, lithium7_tension_ratio_channel_coupled, lithium7_tension_ratio_corrected,
    primordial_lithium7_ratio_channel_coupled, primordial_lithium7_ratio_corrected, BbnWindows,
    LI7H_OBSERVED, LITHIUM7_BE7_CHANNEL_FRACTION, LITHIUM7_BE7_DARK_SUPPRESSION,
    LITHIUM7_CHANNEL_COUPLED_FACTOR, LITHIUM7_DIRECT_CHANNEL_FRACTION,
    LITHIUM7_TENSION_AMPLIFICATION, LITHIUM7_VOID_CORRECTION,
};
...
fn main() {
    ...
    let li7h_baseline = score.li7h_pred;
    let ratio_baseline = score.li_tension_ratio;

    let li7h_corrected = primordial_lithium7_ratio_corrected(score.eta10);
    let ratio_corrected = lithium7_tension_ratio_corrected(score.eta10);
    ...
    .expect("write");
    writeln!(txt, "li7h_pred = {:.12e}", li7h_baseline).expect("write");
    writeln!(txt, "li7_tension_ratio = {:.12}", ratio_baseline).expect("write");
    writeln!(txt).expect("write");
    writeln!(txt, "[corrected]").expect("write");
    ...
    writeln!(txt, "li7h_pred = {:.12e}", li7h_corrected).expect("write");
    writeln!(txt, "li7_tension_ratio = {:.12}", ratio_corrected).expect("write");
    writeln!(txt, "unity_window_0p8_1p2 = {}", (0.8..=1.2).contains(&ratio_corrected))
        .expect("write");
    writeln!(txt).expect("write");
    ...
    writeln!(
        json,
        "{{\n  \"eta10\": {:.12},\n  \"li7_observed\": {:.12e},\n  \"baseline\": {{\"amplification\": {:.12}, \"li7h_pred\": {:.12e}, \"tension_ratio\": {:.12}}},\n  \"corrected\": {{\"occupancy_factor\": {:.12}, \"void_factor\": {:.12}, \"closure_factor\": {:.12}, \"li7h_pred\": {:.12e}, \"tension_ratio\": {:.12}, \"unity_window_0p8_1p2\": {}}},\n  \"channel_coupled\": {{\"direct_fraction\": {:.12}, \"be7_fraction\": {:.12}, \"be7_dark_suppression\": {:.12}, \"channel_factor\": {:.12}, \"li7h_pred\": {:.12e}, \"tension_ratio\": {:.12}, \"unity_window_0p8_1p4\": {}}}\n}}",
        score.eta10,
        LI7H_OBSERVED,
        LITHIUM7_TENSION_AMPLIFICATION,
        li7h_baseline,
        ...
        li7h_corrected,
        ratio_corrected,
        (0.8..=1.2).contains(&ratio_corrected),
    ...
    println!(
        "Li-7 ratio baseline={:.6}, corrected={:.6}, channel_coupled={:.6} (eta10={:.4})",
        ratio_baseline, ratio_corrected, ratio_channel, score.eta10
    );
}
