...
use gutoe_physics::bbn::{
    ...
    lithium7_residual_post_bbn_depletion_factor, lithium7_tension_ratio_channel_coupled,
    ...
    LITHIUM7_DIRECT_CHANNEL_FRACTION, LITHIUM7_REACTION_NETWORK_GAIN,
...
fn main() {
    let out_dir = std::env::var("GUTOE_LITHIUM_OUT")
        .unwrap_or_else(|_| "/tmp/bh_renders/lithium7_report".to_string());
    ...
    writeln!(
        txt,
        "li7_visible_fraction_11_over_16 = {:.12}",
        LITHIUM7_VISIBLE_FRACTION
    )
    .expect("write");
    writeln!(
        txt,
        "li7_reaction_network_gain_33_over_16 = {:.12}",
    ...
    writeln!(
        txt,
        "occupancy_factor_5_over_16 = {:.12}",
        DARK_FRACTION_TOTAL_STATE_SPLIT
    )
    .expect("write");
    writeln!(
        txt,
        "void_factor_66_over_67 = {:.12}",
    ...
    writeln!(
        txt,
        "direct_fraction_1_over_16 = {:.12}",
        LITHIUM7_DIRECT_CHANNEL_FRACTION
    )
    .expect("write");
    writeln!(
        txt,
        "be7_fraction_15_over_16 = {:.12}",
        LITHIUM7_BE7_CHANNEL_FRACTION
    )
    .expect("write");
    writeln!(
        txt,
        "be7_dark_suppression = {:.12}",
        LITHIUM7_BE7_DARK_SUPPRESSION
    )
    .expect("write");
    writeln!(
        txt,
        "channel_coupled_factor = {:.12}",
    ...
    .expect("write");
    writeln!(txt).expect("write");
    writeln!(txt, "[closure_transduction]").expect("write");
    writeln!(
        txt,
        "be7_dark_suppression_structural = {:.12}",
        LITHIUM7_BE7_DARK_SUPPRESSION
    )
    .expect("write");
    writeln!(
        txt,
        "be7_dark_suppression_required_for_unity = {:.12}",
        li7_be7_required_suppression
    )
    .expect("write");
    writeln!(
        txt,
        "be7_destruction_enhancement_structural = {:.12}",
        LITHIUM7_BE7_DESTRUCTION_ENHANCEMENT_STRUCTURAL
    )
    .expect("write");
    writeln!(
        txt,
        "be7_destruction_enhancement_required_for_unity = {:.12}",
        li7_be7_required_enhancement
    )
    .expect("write");
    writeln!(
        txt,
        "be7_additional_destruction_multiplier = {:.12}",
        li7_be7_additional_multiplier
    )
    .expect("write");
    writeln!(
        txt,
        "residual_post_bbn_depletion_factor = {:.12}",
        li7_residual_depletion_factor
    )
    .expect("write");
    writeln!(
        txt,
        "residual_post_bbn_depletion_percent = {:.12}",
